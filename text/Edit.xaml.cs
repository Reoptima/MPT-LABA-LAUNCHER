...
namespace text
{
    ...
    public partial class Edit : Window
    {
        public Edit()
        {
            ...
            cmbFontSize.ItemsSource = new List<double>() { 8, 9, 10, 11, 12, 14, 16, 18, 20, 22, 24, 26, 28, 36, 48, 72 }; // предлогаемые варинты выбора шрифта
        ...
        private void rtbEditor_SelectionChanged(object sender, RoutedEventArgs e) // жирный, курсивный и подчёркнутый текст
        {
            try
            {
            ...
            catch (System.Exception) { } // так надо, чтоб не вылетало из-за сам не знаю чего, но если жать всё подрят - оно вылетит :)
        }
        ...
        private void Save_Executed(object sender, ExecutedRoutedEventArgs e) //сохранить файл
        {
            ...
            try
            {
                ...
                dlg.Filter = "Rich Text Format (*.rtf)|*.rtf|All files (*.*)|*.*";
                if (dlg.ShowDialog() == true) // если файл уже был где-то сохранён, то при нажатии кнопки сохранить, он просто сохранится в том же самом месте.
                {
                    FileStream fileStream = new FileStream(dlg.FileName, FileMode.Create);
                    range.Save(fileStream, DataFormats.Rtf);
                }
            }
            catch (System.Exception) { } // шоб не вылетало с ошибкой "файл уже открыт где-то
        }
        ...
        private void cmbFontSize_TextChanged(object sender, TextChangedEventArgs e) // Размер шрифта
        {
            ...
            catch (System.Exception) { } // чтоб при значении размера шрифта 0 Double не вылетел с криками "Я ТАК НЕ УМЕЮ". Шрифт при этом не будет меняться и останется каким и был.
        }
        ...
        private void IzmButt_Click(object sender, RoutedEventArgs e) // Открыть вкладку изменений
        {
            ...
            IzmButt.Foreground = Brushes.White; // Всё снизу меняет цвет кнопки изменений, чтобы она выглядела как включенная
        ...
        {
            var Text_Color = ClrPcker_Background.SelectedColor.ToString(); // приводим полученный цвет в текстовый формат
            Char[] ColorChar = { '{', '}' }; // из полученного текстового формата в виде например {#FFA500} удаляем скобочки, чтобы Foregraund понял цвет в формате HEX
            string Colorval = Text_Color.TrimEnd(ColorChar); //убираем ковычки из значения цвета
            rtbEditor.Selection.ApplyPropertyValue(TextElement.ForegroundProperty, value: Colorval); // меняем foreground выделенного текста
        }
        ...
        private void Print_Click(object sender, RoutedEventArgs e) // отправить на предпросмотр к печати
        {
            ...
            {
                pd.PrintDocument((((IDocumentPaginatorSource)rtbEditor.Document).DocumentPaginator), "printing as paginator"); // я не знаю что тут и как, но оно работает, почти.
            }
        }
    }
}